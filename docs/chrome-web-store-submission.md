...
本地优先的浏览器工作台，用专注计时、网站屏蔽和时间统计管理浏览器里的打扰。
...
01Kit 是一个本地优先的浏览器工作台，适合想减少分心、了解自己浏览时间的人。
...
01Kit 的单一用途是帮助用户在 Chrome 中管理浏览器里的注意力和本地使用记录。当前功能都围绕这一用途展开：开启专注、屏蔽网站、统计域名停留时间、查看专注记录和导出本地数据。
